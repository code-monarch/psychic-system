...
const CustomLabel = ({ viewBox, label, data }: { viewBox?: any, label: { text: string, color: string }, data: { value: number, color: string } }) => {
    const { cx, cy } = viewBox;
    return (
        <>
...
export function DonutChart<T extends object>({ data, dataKey, valueGetter } : {data: T[], dataKey: string, valueGetter: (datum: T) => number}) {
    ...
    const colors = [ theme.colors.secondary.yellow, theme.colors.secondary.lightgreen, theme.colors.primary.green, theme.colors.secondary.blue, theme.colors.secondary.darkgreen ];
