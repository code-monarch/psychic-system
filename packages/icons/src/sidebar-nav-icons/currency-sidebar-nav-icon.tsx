...
export const CurrencySidebarNavIcon = ({
  // Custom color prop
  color: iconColor,
  width,
  height,
  ...
  // FSP app link
  const FSPHref = "/cbdc-application";
  ...
  // For FSP App
  useEffect(() => {
    if (pathname?.startsWith(FSPHref) && !iconColor) {
      setColor("#C0933E");
    } else if (!pathname?.startsWith(FSPHref) && !iconColor) {
      setColor("#8499B1");
    }
  }, [iconColor, pathname]);
  ...
  return (
    <svg
      ...
      {...props}
    >
      <g clipPath='url(#clip0_317_23559)'>
        <path
